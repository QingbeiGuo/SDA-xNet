...
class BasicStage(nn.Module):
    def __init__(self, block, inplanes, planes, blocks, stride=1, norm_layer=None, pyconv_kernels=[3], pyconv_groups=[1], r=16, L=64):
        super(BasicStage, self).__init__()

        if norm_layer is None:
            norm_layer = nn.BatchNorm2d
        downsample = None
        if stride != 1 and inplanes != planes * block.expansion:
            downsample = nn.Sequential(
                nn.MaxPool2d(kernel_size=3, stride=stride, padding=1),
                conv1x1(inplanes, planes * block.expansion),
                norm_layer(planes * block.expansion),
            )
        elif inplanes != planes * block.expansion:
            downsample = nn.Sequential(
                conv1x1(inplanes, planes * block.expansion),
                norm_layer(planes * block.expansion),
            )
        ...
        layers = []
        layers.append(block(inplanes, planes, stride=stride, downsample=downsample, norm_layer=norm_layer,
...
class PyConvResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False, norm_layer=None, dropout_prob0=0.0):
        ...
        self.stage1 = BasicStage(block, self.inplanes, 64, layers[0], stride=2, norm_layer=norm_layer,
                                       pyconv_kernels=[3, 5, 7, 9], pyconv_groups=[1, 4, 8, 16])
        self.inplanes = 256
        self.stage2 = BasicStage(block, self.inplanes, 128, layers[1], stride=2, norm_layer=norm_layer,
                                       pyconv_kernels=[3, 5, 7], pyconv_groups=[1, 4, 8])
        self.inplanes = 512
        self.stage3 = BasicStage(block, self.inplanes, 256, layers[2], stride=2, norm_layer=norm_layer,
                                       pyconv_kernels=[3, 5], pyconv_groups=[1, 4])
        self.inplanes = 1024
        self.stage4 = BasicStage(block, self.inplanes, 512, layers[3], stride=2, norm_layer=norm_layer,
...
def pyconvresnet50(pretrained=False, **kwargs):
    ...
    model = PyConvResNet(PyConvBlock, [5, 6, 12, 5], **kwargs)
    if pretrained:
